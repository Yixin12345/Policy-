...
const PdfViewer = ({
  ...
  const documentTypePresentation = useMemo(() => {
    ...
    const palette: Record<string, { label: string; colorClass: string }> = {
      cmr_form: { label: 'CMR_Form', colorClass: 'text-sky-600' },
      facility_invoice: { label: 'Facility_Invoice', colorClass: 'text-emerald-600' },
      ub04: { label: 'UB04', colorClass: 'text-amber-600' },
    }
  ...
  const highlightPalette = useMemo<Record<HighlightRegion['section'], { border: string; background: string; labelBg: string; labelText: string }>>(() => ({
    facility_invoice: {
      ...
      background: 'bg-emerald-400/15',
      labelBg: 'bg-emerald-500',
      labelText: 'text-white',
    },
    cmr_form: {
      border: 'border-sky-500/90',
      background: 'bg-sky-400/15',
      labelBg: 'bg-sky-500',
      labelText: 'text-white',
    },
    ub04: {
