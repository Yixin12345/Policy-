...
import { fetchJobHistoryDetail, downloadCanonicalExcel } from '../services/apiClient'
...
const WorkspacePage = () => {
  ...
  const mappingInProgress = canonicalEnabled && canonicalQuery.isFetching
  ...
  const handleExportExcel = async () => {
    ...
    try {
      await downloadCanonicalExcel(jobId)
    } catch (error) {
  ...
  return (
    <AppShell>
      <HeaderBar
        ...
        statusOverrideLabel={mappingInProgress ? 'Mapping' : undefined}
        statusOverrideColor={mappingInProgress ? '#4f46e5' : undefined}
